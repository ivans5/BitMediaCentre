...
{
    TProgram::idle();
    if (mycounter++ % 50 == 0) { //50 is around 5 seconds...
        clock->update();
        if(myTChDirDialog != NULL)  {
...
TMenuBar *TVDemo::initMenuBar(TRect r)
{
    ...
    TSubMenu& sub2 =
      *new TSubMenu( "~F~ile", 0, hcFile ) +
        ...
        *new TMenuItem( "~P~lay File...", cmChDirCmd, kbNoKey, hcFChangeDir ) +
         newLine() +
    ...
    //XXX: generate "YouTube" menu from the scripts in /start/youtube folder:
    DIR *dp;
    dirent *de;
    TSubMenu& sub5 = *new TSubMenu( "~y~outube-dl", 0, hcYoutube );
    int i=0;
    if ((dp = opendir("/start/youtube")) != NULL)
    {
