...
std::map<int, std::string> youtubeScripts;
    ...
    if (event.what == evCommand)
    {
        ...
        case cmRestoreCmd:          //  Restore saved desktop
            ...
            default:                    //  Unknown command
                if (event.message.command>=cmYoutubeCmd)  {
                   std::string x = youtubeScripts[event.message.command];
//             std::cout << x << std::endl ;
char command[2048];                  
sprintf(command,"/start/youtube/%s > /tmp/mpv.log 2> /tmp/mpv.log.stderr |cat", x.c_str());
system(command);
                }
                return;
...
void TVDemo::changeDir()
{
    ...
    myTChDirDialog = NULL; //XXX prevent idle() from trying to update it...
}
